...
{
    public class CounsellorAddNewController : Controller
    {
        private InomiEntities db = new InomiEntities();

        // GET: AddNew
        [SessionExpireAttribute]
        public ActionResult CounsellorAddNew(string StrMain, string tab)
        {
            ...
            dtProjectDetails = StudentCon.ProjectDetails();
            ViewBag.ProjectDetails = dtProjectDetails;
        ...
        public ActionResult InsertStudentStatus(string json, string Id, string jsonIns)
        {
            ...
            StudentCon.InsertStudentDetails(XmlDoc.InnerXml, Id, filePath, XmlInsDate.InnerXml);
        ...
        public ActionResult InsertFormatStatus(string json, string Id, string EmailText, string jsonC, string jsonS, string Emailsubject)
        {
           ...
           // StudentCon.InsertFormatDetails(XmlDoc.InnerXml, Id, filePath, EmailText);
            TempData["Message"] = "Record has been update successfully";
            SendEmail(filePath, fname, Emailbody, XmlDocC.InnerXml, XmlDocS.InnerXml, Emailsubject);
            return RedirectToAction("Student", "AddNew");
        }
        public void InsertProjectStatus(string json, string Id, string jsonDetails)
        {
            string UsertypeId = Session["UserTypeId"].ToString();
            XmlDocument XmlDoc;
            XmlDocument XmlDocDetails;
            XmlDoc = (XmlDocument)JsonConvert.DeserializeXmlNode("{\"Details\":" + json + "}", "Project");
            XmlDocDetails = (XmlDocument)JsonConvert.DeserializeXmlNode("{\"Details\":" + jsonDetails + "}", "Project");
            StudentCon.InsertProjectDetails(XmlDoc.InnerXml, Id, XmlDocDetails.InnerXml, UsertypeId);
            //TempData["Message"] = "Record has been update successfully";
            //return RedirectToAction("Student", "AddNew");
        }
        ...
        public ActionResult DeleteProjectDetails(string id)
        {
            StudentCon.DeleteProjectDetails(id);
            TempData["Message"] = "Record has been delete successfully";
            return RedirectToAction("Student", "AddNew");
        }
        public ActionResult EditProjectDetails(string id)
        {
            var Model = new List<ProjectDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.EditProjectDetails(id);

            if (dt.Rows.Count > 0)
            {

                ProjectDetails projectDetails = new ProjectDetails();
                string Rid = dt.Rows[0]["ProjectID"].ToString();
                int Noi = int.Parse(Rid);

                projectDetails.ProjectID = Noi;
                projectDetails.Student = dt.Rows[0]["StudentID"].ToString();
                projectDetails.Name = dt.Rows[0]["Name"].ToString();
                projectDetails.Description = dt.Rows[0]["Description"].ToString();

                Model.Add(projectDetails);
        ...
        public ActionResult GetEssayDetails(string CID)
        {
            var Model = new List<EssayDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.GetEssayDetails(CID);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i=0; i < dt.Rows.Count; i++)
                {
                    EssayDetails essayDetails = new EssayDetails();
                    essayDetails.EssayName = dt.Rows[i]["EssayName"].ToString();
                    essayDetails.Topic = dt.Rows[i]["Topic"].ToString();
                    essayDetails.WordCount = dt.Rows[i]["WordCount"].ToString();
                    essayDetails.deadline = dt.Rows[i]["deadline"].ToString();

                    Model.Add(essayDetails);
                }
        ...
        public ActionResult GetScoreDetails(string CID)
        {
            var Model = new List<EssayDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.GetScoreDetails(CID);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    EssayDetails essayDetails = new EssayDetails();
                    essayDetails.Type = dt.Rows[i]["PId"].ToString();
                    essayDetails.Score = dt.Rows[i]["PScore"].ToString();
        ...
        public ActionResult GetProjectDetails(string CID)
        {
            var Model = new List<ProjectDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.GetProjectDetails(CID);
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    ProjectDetails projectDetails = new ProjectDetails();
                    projectDetails.Milestone = dt.Rows[i]["Milestone"].ToString();
                    projectDetails.deadline = dt.Rows[i]["Deadline"].ToString();
                    projectDetails.Define = dt.Rows[i]["Define"].ToString();
        ...
        public ActionResult GetEssayName(string Cid)
        {
            var Model = new List<CityDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.EssayDetails(Cid);
        ...
        public ActionResult GetEssayCount(string Cid)
        {
            var Model = new List<CityDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.EssayCount(Cid);
        ...
        public ActionResult MileStoneDetails(string id)
        {
            var Model = new List<ProjectDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.GetMileStoneDetails(id);

            if (dt.Rows.Count > 0)
            {

                ProjectDetails projectDetails = new ProjectDetails();
                string Rid = dt.Rows[0]["MID"].ToString();
                int Noi = int.Parse(Rid);

                projectDetails.Id = Noi;
                projectDetails.Define = dt.Rows[0]["Define"].ToString();
                projectDetails.Upload = dt.Rows[0]["Upload"].ToString();
        ...
        public void MileStoneAction(string id)
        {
            var Model = new List<ProjectDetails>();
            DataTable dt = new DataTable();
            dt = StudentCon.MileStoneAction(id);
